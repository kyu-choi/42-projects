...
/**
 * @file mini_rt.h
 * @brief Mandatory target: public types and prototypes for miniRT
 *
 * Ray tracer: parse .rt scene, render sphere/plane/cylinder with ambient
 * and directional light. Uses MiniLibX for display. Validation fails fast
 * with readable rt_error messages.
 *
 * @section macros Macro groups
 * - WIN_W, WIN_H: window size
 * - EPS, STEP_*: numeric tolerance and interaction step
 * - OBJ_*: object type IDs (sphere, plane, cylinder)
 * - KEY_*, EVENT_*: input key codes and window events
 *
 * @section types Type groups
 * - Geometry: t_vec3, t_ray, t_color
 * - Scene: t_obj, t_ambient, t_light, t_camera, t_scene
 * - Render: t_hit, t_img, t_rt
 */

/* --- Window and numeric constants --- */
# define WIN_W 1280
# define WIN_H 720
# define EPS 1e-6
...
typedef struct s_ray /* origin + direction (unit) */
{
	t_vec3	orig;
	t_vec3	dir;
}	t_ray;
...
/**
 * @defgroup error Error reporting and resource cleanup
 * @brief Write errors to stderr; free scene/objs; exit handler.
 * @{
 */
...
/**
 * @defgroup vec3 3-D vector arithmetic
 * @brief Construct, add, sub, scale, dot, len, norm, cross, negate.
 * @{
 */
...
/**
 * @defgroup color Color arithmetic (0-1 float range)
 * @brief Construct, add, scalar-mul, modulate, convert to 0xRRGGBB.
 * @{
 */
...
/**
 * @defgroup parse Scene file (.rt) parser
 * @brief Read file, tokenize lines, parse A/C/L/sp/pl/cy elements.
 *
 * Parse pipeline: rt_read_file -> rt_parse_text (line-by-line)
 *  -> rt_parse_line -> rt_dispatch -> element-specific parser.
 * @{
 */
...
/**
 * @defgroup scene Scene object linked-list management
 * @{
 */
...
/**
 * @defgroup init Initialization, hooks, and interactive controls
 * @brief MLX setup, key/expose handlers, camera/scene movement.
 * @{
 */
...
/**
 * @defgroup render Pixel write, full-frame trace, UI overlay
 * @{
 */
...
/**
 * @defgroup ray Ray tracing pipeline
 * @brief Camera ray generation, intersection tests, shadow,
 *        Lambertian shading, and top-level trace.
 * @{
 */
