...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/05/24 20:38:43 by kyu-choi          #+#    #+#             */
/*   Updated: 2025/05/24 20:39:18 by kyu-choi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	res = first_remain(&remain);
	if (res)
		return (res);
	count = 0;
	temp = NULL;
	res = whilestr(&remain, fd, temp, count);
	if (res)
		return (res);
	return (final_remain(&remain));
}
