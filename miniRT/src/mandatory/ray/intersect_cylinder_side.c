...
/**
 * @file intersect_cylinder_side.c
 * @brief Ray–cylinder lateral surface intersection.
 *
 * An infinite cylinder with axis A and radius r satisfies
 *   |P − C − ((P−C)·A)A|² = r².
 * Substituting P(t) = O + tD and expanding yields a quadratic in t.
 * After solving, each root is accepted only if the axial projection
 * falls within ±height/2 (finite cylinder clipping).
 */
...
/**
 * @brief Fills the hit record for a lateral-surface intersection.
 *
 * The outward normal is computed by subtracting the axial component
 * from the center-to-point vector, then normalising.  The normal is
 * flipped if it faces away from the ray (inside hit).
 *
 * @param obj Object (cylinder).
 * @param ray Incoming ray.
 * @param t   Parameter at the hit point.
