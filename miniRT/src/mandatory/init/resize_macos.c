...
/**
 * @file resize_macos.c
 * @brief macOS window resize support via Objective-C runtime.
 *
 * MinilibX on macOS wraps an NSWindow but does not expose a resize
 * API.  This file reaches into the Cocoa layer through the ObjC
 * runtime (objc_msgSend) to toggle the NSWindowStyleMaskResizable
 * bit and poll the content-view frame size each loop iteration.
 *
 * Key helpers:
 *   rt_msg_id    – sends a selector that returns an ObjC id.
 *   rt_msg_ulong – sends a selector that returns unsigned long.
 *   rt_view_size – reads the NSView frame rectangle.
 *
 * @note Linux builds use resize_linux.c instead (no-op stubs).
 */
