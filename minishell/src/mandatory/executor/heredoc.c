			...
			int expand, t_shell *shell)
{
	...
	if (expand)
	{
		expanded = expand_heredoc_variables(line, shell);
...
/**
 * @brief Writes single heredoc line to pipe
 * @param fd File descriptor to write to
 * @param line Input line to write
...
/**
 * @brief Reads single line for heredoc
 * @return Line read (newly allocated), NULL on EOF
 */
